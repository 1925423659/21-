...
with tensorflow.Session() as sess:
    ...
    # 读入的都是训练图像
    filenames = [os.path.join('dataset/cifar10/cifar-10-batches-bin', 'data_batch_%d.bin' % i) for i in range(1, 6)]
    # 判断文件是否存在
    for f in filenames:
    ...
    read = cifar10_input.read_cifar10(filename_queue)
    # 将图片转换为实数形式
    # 返回的 reshaped_image 是一张图片的 tensor
    # 应当这样理解 reshaped_image：每次使用 sess.run(reshaped_image)，就会取出一张图片
    reshaped_image = tensorflow.cast(read.uint8image, tensorflow.float32)
